...
export default function CartDetailsModal({ cartData, setCartData, onClose }) {
  function handleDeleteItem(item) {
    const newCart = cartData.filter((cartItem) => cartItem.id !== item.id);
    setCartData(newCart);
  }
              ...
              cartData.map((item) => (
                <div key={item.id} className="grid grid-cols-[1fr_auto] gap-4">
                  ...
                  <div className="flex justify-between gap-4 items-center">
                    ...
                    >
                      <img className="w-5 h-5" src={delete_icon} alt="delete" />
